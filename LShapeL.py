...
#  x
#  x
class LShapeL(Tetrino):
    def __init__(self, block_side, length, width):
        # positions = [Position(0, 18), Position(1, 18), Position(2, 18), Position(2, 19)]
        middle_col = length // 2
        positions = [Position(middle_col - 1, width - 2), Position(middle_col, width - 2),
                     Position(middle_col + 1, width - 2), Position(middle_col + 1, width - 1)]
        super().__init__(block_side, positions)

    def _set_direction(self, board_length, board_width, direction, board_status):
        new_blocks = []
        center_pos = self._blocks[1].pos
        positions = []
        if direction == 0:
            positions.append(Position(center_pos.col - 1, center_pos.row))
            positions.append(center_pos)
            positions.append(Position(center_pos.col+1, center_pos.row))
            positions.append(Position(center_pos.col + 1, center_pos.row+1))
